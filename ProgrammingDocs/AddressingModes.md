...
In this mode, the value of the register is automatically decremented (by 1 for byte operations, by 2 for word operations) *before* the data is accessed. The resulting address is then used to access the operand.
...
*   **Example**: `LDI R1, my_data` - Loads the *address* of `my_data` into `R1`.
